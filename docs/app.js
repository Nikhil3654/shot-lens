...
function renderModelsView() {
  setControls("");
  setSummary([]);
  const modelTables = [`<div class="panel"><h3>Shot Model Metrics</h3>${table(state.metrics, Object.keys(state.metrics[0] || {}))}</div>`];
  if (state.breakoutMetrics.length) modelTables.push(`<div class="panel"><h3>Breakout Model Metrics</h3>${table(state.breakoutMetrics, Object.keys(state.breakoutMetrics[0] || {}))}</div>`);
  setContent(modelTables.join(""));
}
